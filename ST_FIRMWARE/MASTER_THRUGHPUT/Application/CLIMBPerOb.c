...
#define DEFAULT_CONNECTABLE_ADVERTISING_INTERVAL          85
#define DEFAULT_NON_CONNECTABLE_ADVERTISING_INTERVAL          85
...
// Scan duration in ms
#define DEFAULT_SCAN_DURATION                 10000 //Tempo di durata di una scansione, allo scadere la scansione viene ricominciata

// Scan interval value in 0.625ms ticks
#define SCAN_INTERVAL 						  32
...
static uint8 led_cycle_counter = 0;
...
static uint8 defScanRspData[] = {
		0x02,   // length of this data
		GAP_ADTYPE_FLAGS,
		DEFAULT_DISCOVERABLE_MODE | GAP_ADTYPE_FLAGS_BREDR_NOT_SUPPORTED, 0x03, // length of this data including the data type byte
...
static void SimpleBLEPeripheral_init(void)
{
  ...
  {
    uint16_t advInt = DEFAULT_CONNECTABLE_ADVERTISING_INTERVAL;

    GAP_SetParamValue(TGAP_CONN_ADV_INT_MIN ,    advInt);
    GAP_SetParamValue(TGAP_CONN_ADV_INT_MAX ,    advInt);

    advInt = DEFAULT_NON_CONNECTABLE_ADVERTISING_INTERVAL;
  ...
  // Setup the GAP Bond Manager
  {
    ...
    uint8_t pairMode = GAPBOND_PAIRING_MODE_WAIT_FOR_REQ;//GAPBOND_PAIRING_MODE_NO_PAIRING;//GAPBOND_PAIRING_MODE_WAIT_FOR_REQ;
    uint8_t mitm = FALSE;
...
static void Climb_contactsCheckSendThroughGATT(void)
{
	...
	while (node != NULL && i < 17) {
		if (node->device.lastContactTicks >= lastGATTCheckTicks) { //invia solo i nodi visti dopo l'ultimo check
			//childrenNodesData[i++] = node->device.devRec.addr[1];
			childrenNodesData[i++] = node->device.devRec.addr[0];
			childrenNodesData[i++] = node->device.advData[12];
...
static void Climb_advertisedStatesUpdate(void) {
	...
	uint8 i = 12;
	uint8 newChildrenStatesData[32];
	...
	while (node != NULL && i < 30) {
		//newChildrenStatesData[i++] = node->device.devRec.addr[1];
		newChildrenStatesData[i++] = node->device.devRec.addr[0];
...
static void Climb_processCharValueChangeEvt(uint8_t paramID) {
	...
	case CLIMBPROFILE_CHAR2:
	{
		...
		throughputTest = newValue[0];
/*
		uint8 i = 0;
		while (i < 18) {
			uint8 nodeID[] = { newValue[i] };//{ newValue[i+1], newValue[i] };
			...
			if (node != NULL) {
				...
				if(newValue[i + 1] != node->device.advData[12] && (ChildClimbNodeStateType_t)newValue[i + 1] != ALERT){ //PER ORA NON BROADCASTARE LO STATO ALERT ALTRIMENTI SE C'E' UN FALSO ALERT IL CHILD VA IN ALERT ANCHE SE NON DOVREBBE

					node->device.stateToImpose = (ChildClimbNodeStateType_t) newValue[i + 1]; //quando uno di questi viene cambiato aggiorna l'adv
					advUpdateReq = TRUE;
	...
	if (gapDeviceInfoEvent_a->eventType == GAP_ADRPT_ADV_SCAN_IND |
		...
		while (index < gapDeviceInfoEvent_a->dataLen) {
			if (gapDeviceInfoEvent_a->pEvtData[index + 1]
				...
				}else if(memcomp(&(gapDeviceInfoEvent_a->pEvtData[index + 2]),&(advertData[2]),(gapDeviceInfoEvent_a->pEvtData[index]) - 1) == 0 ){ //TODO: verificare...
...
static listNode_t* Climb_findChildNodeById(uint8 *nodeID){
	...
	while (node != NULL) {
		if (*nodeID == node->device.devRec.addr[0]){//memcomp(nodeID, node->device.devRec.addr, NODE_ID_LENGTH) == 0) {
			return node; //se trovi il nodo esci e passane il puntatore
		}
		node = node->next; //passa al nodo sucessivo
	}
...
static listNode_t* Climb_addNode(gapDeviceInfoEvent_t *gapDeviceInfoEvent, ClimbNodeType_t nodeType){
	...
	listNode_t* new_Node_Ptr = (listNode_t*) malloc(sizeof(listNode_t));
...
static void Climb_advertisedStatesCheck(void)
{
	...
	while (node != NULL && i < 17) {
		...
		if (node->device.advData[12] == BY_MYSELF ) { //accetta automaticamente tutti i nodi climb!
			node->device.stateToImpose = CHECKING;
		}

		//ACCETTA AUTOMATICAMENTE TUTTI I NODI CLIMBC E FALLI ANDARE IN CHECKIN
		if (node->device.advData[12] != node->device.stateToImpose) {
			advUpdateReq = TRUE;
		}
...
static void Climb_nodeTimeoutCheck() {
	...
	while (targetNode != NULL) {
		...
		if (nowTicks - targetNode->device.lastContactTicks> NODE_TIMEOUT_OS_TICKS) {
			//ATTENZIONE, SE UN NODO E' ON BOARD, ALLO SCADERE DEL TIMEOUT NON DEVE ESSERE ELIMINATO MA ANDARE IN ALLERTA (per ora non lo faccio)!!!
...
#endif
	free(nodeToRemove); //rilascia la memoria
}
...
static void blastData(void)
{
  ...
  while(status == SUCCESS)
  {
    ...
    {
      //place index
      noti.pValue[3] = (msg_counter >> 24) & 0xFF;
      noti.pValue[2] = (msg_counter >> 16) & 0xFF;
      noti.pValue[1] = (msg_counter >> 8) & 0xFF;
      noti.pValue[0] = msg_counter & 0xFF;
