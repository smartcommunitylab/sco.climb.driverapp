...
bStatus_t GAPRole_StartDevice(gapRolesCBs_t *pAppCallbacks)
{
  ...
  {   
    ...
    // Start the GAP
    bStatus_t status = gapRole_SetupGAP();
...
{ 
  ...
  Util_constructClock(&startAdvClock, gapRole_clockHandler,0, 0, false, START_ADVERTISING_EVT);
  Util_constructClock(&startUpdateClock, gapRole_clockHandler,0, 0, false, START_CONN_UPDATE_EVT);
  Util_constructClock(&updateTimeoutClock, gapRole_clockHandler, 0, 0, false, CONN_PARAM_TIMEOUT_EVT);
