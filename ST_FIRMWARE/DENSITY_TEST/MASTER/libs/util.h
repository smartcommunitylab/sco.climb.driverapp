 ...
 * @fn      Util_constructClock
 *
 * @brief   same as Util_constructClock but arguments are in microseconds
 *
 * @param   pClock        - pointer to clock instance structure.
 * @param   clockCB       - callback function upon clock expiration.
 * @param   clockDuration - longevity of clock timer in microseconds.
 * @param   clockPeriod   - duration of a periodic clock, used continuously
 *                          after clockDuration expires.
 * @param   startFlag     - TRUE to start immediately, FALSE to wait.
 * @param   arg           - argument passed to callback function.
 ...
 * @return  Clock_Handle  - a handle to the clock instance.
 */
extern Clock_Handle Util_constructMicroClock(Clock_Struct *pClock,
                                        Clock_FuncPtr clockCB,
                                        uint32_t clockDuration,
                                        uint32_t clockPeriod,
                                        uint8_t startFlag,
 ...
 * @fn      Util_setClockTimeout
 *
 * @brief   Restart a micro clock by changing the timeout.
 *
 * @param   pClock - pointer to clock struct
 * @param   clockTimeout - longevity of clock timer in microseconds
 *
 * @return  none
 */
extern void Util_restartMicroClock(Clock_Struct *pClock, uint32_t clockTimeout);
/*********************************************************************
 * @fn      Util_isActive
 *
 ...
 *
 * @param   pClock - pointer to clock struct
 * @param   clockPeriod - longevity of clock timer in microseconds
 * @return  none
 */
extern void Util_rescheduleMicroClock(Clock_Struct *pClock, uint32_t clockPeriod);
