 ...
 * @return  Clock_Handle  - a handle to the clock instance.
 */
Clock_Handle Util_constructMicroClock(Clock_Struct *pClock,
                                 Clock_FuncPtr clockCB,
                                 uint32_t clockDuration,
                                 uint32_t clockPeriod,
                                 uint8_t startFlag,
                                 UArg arg)
{
  ...
  // Convert clockDuration in milliseconds to ticks.
  uint32_t clockTicks = clockDuration / Clock_tickPeriod;
  ...
  // If period is 0, this is a one-shot timer.
  clockParams.period = clockPeriod  / Clock_tickPeriod;
...
void Util_restartMicroClock(Clock_Struct *pClock, uint32_t clockTimeout)
{
  uint32_t clockTicks;
  Clock_Handle handle;
  ...
  // Convert timeout in microseconds to ticks.
  clockTicks = clockTimeout / Clock_tickPeriod;
...
void Util_rescheduleMicroClock(Clock_Struct *pClock, uint32_t clockPeriod)
{
  bool running;
  uint32_t clockTicks;
  Clock_Handle handle;
  ...
  // Convert period in milliseconds to ticks.
  clockTicks = clockPeriod  / Clock_tickPeriod;
